...
- デバッグ情報: 行番号に対応するコード領域の位置が格納されています。
...
|  2 | TYPE_STRING    | 文字列(code = データ領域のオフセット) |
|  3 | TYPE_DNUM      | 実数値(code = データ領域のオフセット) |
|  4 | TYPE_INUM      | 整数値(code = 値) |
|  5 | TYPE_STRUCT    | モジュール変数・構造体(code = minfoID) |
|  6 | TYPE_XLABEL    | 未使用 |
|  7 | TYPE_LABEL     | ラベル名(code = ラベル情報のオフセット) |
...
- パラメーターが変数や固定値だった場合は値をスタックに積みます。(TYPE_VAR、TYPE_INUMなど)
...
中間コードで、`TYPE_CMPCMD`のタイプを持つコードは、比較命令を示しています。このタイプのCode値は、0の場合if命令を、1の場合else命令を示します。比較命令の場合に限り、中間コードの直後に16bitのオフセットが埋め込まれています。オフセットは、比較命令が正しくなかった時(if命令以降をスキップする場合)のコード領域内ジャンプ先が相対値として記録されています。


## デバッグ情報

デバッグ情報にはスクリプトのデバッグに関する情報が格納されています。主にランタイム上でエラーの表示やデバッグウィンドウ上での表示補助に利用されます。デバッグ情報はリリース時には含まれません。

デバッグ情報のデータは符号なし8bitデータの列が続いています。
...
| 254 | ソースファイル指定(*1) |
| 253 | 変数名データ指定(*2) |
| 252 | 次の16bit値が、次の行までのコード領域のオフセット |
| その他 | 次行までのCSオフセット |
...
このデータは変数IDの順にデバッグ情報領域内に連続して設定されている。
